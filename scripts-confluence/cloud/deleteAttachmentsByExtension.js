...
javascript:
function main() {
    var fileType = prompt('Enter valid file types(jpeg, zip, mp4, png, mov, csv):');
    var fileType = fileType.replace(/\\s+/g, '');
    if (!fileType.includes(',')) {
        fileType = fileType + ',';
        fileType = fileType.split(',');
        fileType = fileType.filter(e => e);
    }
    if (fileType.includes(',')) {
        fileType = fileType.split(',');
        fileType = fileType.filter(e => e);
    ...
    fileType.forEach(item => {
        if(item[0] == '.') {
        var indx = fileType.indexOf(item);
    ...
    const repo = [];
    const count = [0];
    var cols = ['No.', 'Attachment ID', 'Title', 'Type', 'Media Type', 'Status', 'Space'];
    ...
    function saveTextAsFile() {
        var textToWrite = document.body;
        var textFileAsBlob = new Blob([textToWrite.outerHTML], {type:'text/plain'});
        var fileNameToSaveAs = `${document.getElementById('para3').innerText}.html`;
    ...
    const fetchData = async (purgeTable) => {
        while(true) {
            ...
            if ( response._links.hasOwnProperty('next') == false ) {
                ...
                para3.innerText = `${pageID}`;
                para3.setAttribute('id', 'para3');
                para3.setAttribute('hidden', 'true');
                ...
                sc2.innerHTML = saveTextAsFile;
                btn1.setAttribute('onclick', 'deleteIds()');
                para2.appendChild(sc1);
                para2.appendChild(sc2);
                para2.appendChild(btn1);
                var btn0 = document.createElement('button');
                btn0.innerText = 'Export to HTML';
                btn0.setAttribute('onclick', 'saveTextAsFile()');
                para2.appendChild(btn0);
